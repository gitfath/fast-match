...
// Helper functions
export const getOptionLabel = (category: keyof typeof PROFILE_OPTIONS, value: string): string => {
    const options = PROFILE_OPTIONS[category];
    if (Array.isArray(options) && options.length > 0) {
        if (typeof options[0] === 'object' && 'label' in options[0]) {
            const option = options.find((opt: any) => opt.value === value);
            return option ? (option as any).label : value;
        }
    }
    return value;
};

export const isValidOption = (category: keyof typeof PROFILE_OPTIONS, value: string): boolean => {
    const options = PROFILE_OPTIONS[category];
